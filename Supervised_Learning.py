...
def manual_assignment():
    ...
    feature_1, feature_2 = 'CYLINDERS', 'FUEL_CONSUMPTION'
...
# Returns R^2 values.
def print_correlation(scores: list[int], original_r=None) -> None:
    
    print(f'Original R^2 score: {original_r}')
    print(f'Cross Validation R^2 scores: {scores}')
    print(f'Average CV R^2 Score: {np.mean(scores).round(2)}.\nStd Dev R^2 Score: {np.std(scores).round(2)}.')
...
def lin_reg(df: pd.DataFrame, feature_1: str, feature_2: str) -> None:
    ...
    # Print R & CV values. 
    print("Standard Linear Regression Values: ")
    print_correlation(r_squared_scores, original_r=r2_score(y_true=y_test,y_pred=y_predict))
...
# Calculates the Lasso Regression: 
# Loss function = OLS + ( alpha * sum( abs( coef(i) ) ) )   
def lin_reg_lasso(df: pd.DataFrame, feature_1: str, feature_2: str) -> None:
...
# Analyses significant features in the dataset with respect to the target (feature_2). 
# Numeric features only   
def significant_features(df: pd.DataFrame, feature_2: str) -> None: 
    
    # Extracts all the numeric feature's data
    X = df.drop(feature_2, axis=1).select_dtypes([np.number]).values
    y = df[feature_2].values
    
    # Extracts all the numeric features
    features = df.drop(feature_2, axis=1).select_dtypes([np.number]).columns
    
    lasso = Lasso(alpha=0.1)
    lasso_coefs = lasso.fit(X, y).coef_
    
    plt.bar(features, lasso_coefs)
    plt.title(f'Numeric Features Significant to {feature_2}')
...
def main(df: pd.DataFrame, feature_1: str, feature_2: str) -> None:
    ...
    if validate_categories(df, feature_1, feature_2):
        ...
        if (get_category_type(df, feature_1, feature_2, 'and')): 
            ...
            # lin_reg_lasso(df, feature_1, feature_2)
            
            # significant_features(df, feature_2)
